...
import aurelienribon.bodyeditor.ui.AutoTracePropertiesDialog;
import aurelienribon.bodyeditor.ui.FileChoosers;
    ...
    private void initializeOtherLabelsCallbacks() {
        lblSetImage.setCallback(new Label.TouchCallback() {
            ...
            public void touchDown(Label source) {
                ...
                FileChooser fileChooser = FileChoosers.buildOpenFileChooser(file -> {
                    ...
                    String imagePath = Ctx.io.buildImagePath(loadedFile);
                    Ctx.bodies.getSelectedModel().setImagePath(imagePath);
                }, typeFilter, FileChooser.SelectionMode.FILES);
                fileChooser.setDirectory(Ctx.io.getProjectDir());
                fileChooser.getTitleLabel().setText("Select the background image for the selected model");
                canvas.stage.addActor(fileChooser.fadeIn());
            }
        });
        ...
        lblAutoTrace.setCallback(new Label.TouchCallback() {
            ...
            public void touchDown(Label source) {
                if (Ctx.io.getProjectFile() == null) return;
                RigidBodyModel model = Ctx.bodies.getSelectedModel();
                if (model == null || !isImageValid() || !model.getShapes().isEmpty()) return;
                AutoTracePropertiesDialog dialog = new AutoTracePropertiesDialog("Auto-trace parameters", result -> {
                    if (result == Boolean.TRUE) {
                        autoTrace();
                    }
                });
                canvas.stage.addActor(dialog.fadeIn());
            }
        });
